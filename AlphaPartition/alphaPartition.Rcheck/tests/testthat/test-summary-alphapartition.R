context("test-summary-alphapartition")

test_that("Test summary.alphaPartition", {
  ...
  ## Test the direct use of by group analysis in the alphaPartition function
  ped$gen <- factor(ped$gen)
  tmp1 <- summary(alphaPartition(x=ped, colAGV=c("trt1", "trt2")), by="gen")
  tmp2 <- alphaPartition(x=ped, colAGV=c("trt1", "trt2"), colBy="gen")
