...
#' \item{father and/or mother can be unknown (missing) - defined with any "code" that is different from existing identifications}
#' }
#'
#' Unknown (missing) values for additive genetic values are propagated down the pedigree
#' so provide all available values from genetic evaluation. Another option is
#' to cut pedigree links - set parents to unknown and remove them from pedigree
#' prior to using this function - see \code{\link[alphaPartition]{pedSetBase}} function.
...
#' group, i.e., \code{summary(alphaPartition(...), by="group")}. See \code{\link[alphaPartition]{summary.alphaPartition}}
...
#' \code{\link[alphaPartition]{pedSetBase}} for setting base population,
#' \code{\link[alphaPartition]{pedFixBirthYear}} for imputing unknown (missing) birth years,
...
#' must be used if identifications in \code{x} are not already given as \code{1:n}
...
#' @param colMid Numeric or character, position or name of a column holding mother identification or
#' maternal grandparent identification if \code{pedType="IPG"} .
...
#'
#' @example inst/examples/examples_alphaPartition.R
#'
...
alphaPartition <- function #
  ### TODO: add alphaPartition description???
(
  x,
  pathNA=FALSE,
  recode=TRUE,
  ...
  ## TODO: move AGV to another object (to simplify work with McMC or some other
  ...
  ## Groups
  if(groupSummary) {
    ## TODO: what does summary() do with NAs?
    test <- is.na(x[, colBy])
    if(any(test)) {
      if(pathNA) {
  ...
  if(!groupSummary) {
    ...
    tmp$xa <- .Call("alphaPartitionDropGroup", 
                 c1_=c1, c2_=c2,
  ...
  class(ret) <- c("alphaPartition", class(ret))
  if(groupSummary) {
    ret$by <- colBy
    ret$N <- N
  ...
  ##value<< An object of class \code{alphaPartition}, which can be used in further analyses - there is a handy summary
  ## method (\code{\link[alphaPartition]{summary.alphaPartition}} works on objects of \code{alphaPartition} class) and a plot method
  ## for its output (\code{\link[alphaPartition]{plot.summaryAlphaPartition}} works on objects of \code{summaryalphaPartition} class).
  ## Class \code{alphaPartition} is a list. The first \code{length(colAGV)} components (one for each trait and named with
  ...
  ##
  ## If \code{colBy!=NULL} the resulting object is of a class \code{summaryalphaPartition}.
  ## See \code{\link[alphaPartition]{summary.alphaPartition}} for details.
  ##
