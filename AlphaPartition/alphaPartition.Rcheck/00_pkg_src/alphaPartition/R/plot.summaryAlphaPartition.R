...
#' graphics.
#'
#' @param x summaryAlphaPartition, object from the \code{alphaPartition(...)} or \code{summary(alphaPartition(...), ...)} call.
...
#' @param useDirectLabelsXlim Numeric, if \code{useGgplot2=FALSE} and \code{xlim=NULL} stretch x-axis limits.
#' @param useGgplot2 Logical, create plots with ggplot2 or lattice.
...
#'
#' @example inst/examples/examples_plotSummaryAlphaPartition.R
#'
...
(
  ...
  useDirectLabelsXlim=c(0.05, 0.075),
  useGgplot2=TRUE,
  labelPath=NULL,
  ...
) {
...
'
  if(useGgplot2) {
    requireNamespace("ggplot2")      ## for plotting;)
  } else {
      requireNamespace("lattice")      ## for plotting;)
  }

  if(useDirectLabels) {
    requireNamespace("directlabels") ## for direct.label()
    requireNamespace("quadprog")     ## for last.qp method
  }
'
  ...
  if(!missing(color)) {
    ...
    if(!useGgplot2) color <- color[color != "#FFFF33"] ## remove yellow color(s)
  }
  ...
  for(rel in c(FALSE, TRUE)) { ## loop over absolute and relative plots
    ...
    for(i in 1:nT) { ## loop over traits
      ...
      if(useGgplot2) { ## using ggplot2
        ...
        if(is.null(xlim)) {
          xlimI <- range(tmp$by,    na.rm=TRUE)
          z <- (xlimI[2] - xlimI[1])
          xlimI[1] <- xlimI[1] - useDirectLabelsXlim[1] * z
          xlimI[2] <- xlimI[2] + useDirectLabelsXlim[2] * z
        } else {
          if(is.list(xlim)) {
            xlimI <- xlim[[i]]
          } else {
        ...
        if(is.null(ylimT)) {
          ylimI <- range(tmp$trait, na.rm=TRUE)
          z <- 0.05 * (ylimI[2] - ylimI[1])
          ylimI[1] <- ylimI[1] - z
          ylimI[2] <- ylimI[2] + z
        } else {
          if(is.list(ylimT)) {
