context("test-alphapartition")

test_that("Test input for alphaPartition ped", {

  ## Example - not really interesting but set up in a way that we can test behaviour of alphaPartition()
  ped <- data.frame(
  ...
  ## .. to test that colAGV columns are not numeric
  ped5 <- ped
  ...
  expect_error(alphaPartition(x=ped[, c("id", "fid", "mid", "pat", "trt1", "trt2")]))

  ## Error if recode=FALSE and input is not numeric
  expect_error(alphaPartition(x=ped[, c("id", "fid", "mid", "pat", "trt1", "trt2")], pathNA=TRUE, recode=FALSE, verbose=0))
  ...
  pedX <- ped
  pedX[1, "trt1"] <- NA
  expect_error(alphaPartition(x=pedX[, c("id", "fid", "mid", "pat", "trt1", "trt2")], pathNA=TRUE, verbose=0))
})

test_that("Test the output of alphaPartition function", {
  ped <- data.frame(
        id=c( "A",   "B",   "C",   "F",   "G",   "J",   "K",   "E",   "D",   "I",   "H",   "L",   "M"),
       fid=c(  NA,    NA,    NA,   "A",   "C",   "F",   "F",   "A",   "A",   "A",   "C",   "K",    NA),
       mid=c(  NA,    NA,    NA,   "B",   "B",   "G",   "G",   "B",   "B",   "B",   "B",   "H",   "H"),
  ...
  ret   <- alphaPartition(x=ped[, c("id", "fid", "mid", "pat", "trt1", "trt2")],  pathNA=TRUE, verbose=0)
  ret2  <- alphaPartition(x=ped,                                                  pathNA=TRUE, verbose=0, colId=1,     colFid=2,      colMid=3,      colPath=6,     colAGV=c(7, 9))
  ret3  <- alphaPartition(x=ped,                                                  pathNA=TRUE, verbose=0, colId="id",  colFid="fid",  colMid="mid",  colPath="pat", colAGV=c("trt1", "trt2"))
  ped$idI <- ped$id
  ret3a <- alphaPartition(x=ped,                                                  pathNA=TRUE, verbose=0, colId="idI", colFid="fid",  colMid="mid",  colPath="pat", colAGV=c("trt1", "trt2"))
  ped$idI <- NULL
  ret4  <- alphaPartition(x=ped[, c("id", "fid", "mid", "pat", "trt1", "trt2")],  pathNA=TRUE, verbose=0, colId="id",  colFid="fid",  colMid="mid",  colPath="pat", colAGV=c("trt1", "trt2"))

  ## ... to test recode argument
  ret5  <- alphaPartition(x=ped3,                                                 pathNA=TRUE, verbose=0, colId="idI", colFid="fidI", colMid="midI", colPath="pat", colAGV=c("trt1", "trt2"))
  ret6  <- alphaPartition(x=ped3,                                                 pathNA=TRUE, verbose=0, colId="idI", colFid="fidI", colMid="midI", colPath="pat", colAGV=c("trt1", "trt2"), recode=FALSE)
  ## ... to test recode and unknown argument
  ret7  <- alphaPartition(x=ped3,                                                 pathNA=TRUE, verbose=0, colId="idI", colFid="fidI", colMid="midI", colPath="pat", colAGV=c("trt1", "trt2"), recode=FALSE, unknown=99)
...
test_that("Test computation", {
  ...
  ## ret   <- alphaPartition(x=ped[, c("id", "fid", "mid", "pat", "trt1", "trt2")], pathNA=TRUE, verbose=2)
  ## Gene flow (T)
  ##  [1,] 1.000 .    .     .    .    . .   . . . .   . .
  ##  [2,] .     1.00 .     .    .    . .   . . . .   . .
  ##  [3,] .     .    1.000 .    .    . .   . . . .   . .
